...
export const TableConstraints: React.FC<TableConstraintsProps> = ({ tableId }) => {
  ...
  const [dropDialog, setDropDialog] = useState<{ constraintId: string; name?: string } | null>(
    null
  );
  ...
  const getConstraintDisplay = (constraint: Constraint): string => {
    switch (constraint.type) {
      case "primary_key":
        return `PRIMARY KEY (${constraint.columns
          .map((colId) => {
      ...
      case "foreign_key": {
        const childColumns = constraint.columns
          .map((colId) => {
            const col = table.columns.find((c) => c.id === colId);
            return col?.name || colId;
          })
        ...
        const parentColumns = (constraint.parentColumns || [])
          .map((colId) => {
            if (parentTableInfo) {
    ...
    <div className="table-constraints-section">
      ...
      {constraints.length === 0 ? (
        <div className="empty-constraints">
          ...
          <p className="hint">
            Add PRIMARY KEY, FOREIGN KEY, or CHECK constraints (informational only in Unity
            Catalog).
          </p>
        </div>
      ) : (
              ...
              <tr key={constraint.id}>
                ...
                <td className="actions-cell">
                  ...
                  <VSCodeButton
                    appearance="icon"
                    onClick={() =>
                      setDropDialog({ constraintId: constraint.id, name: constraint.name })
                    }
      ...
      <div className="constraint-actions">
        <button className="add-constraint-btn" onClick={handleAddConstraintClick}>
          + Add Constraint
        </button>
      </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
                  ...
                  <div className="constraint-columns-list" role="group" aria-label="Select columns">
                    <div className="constraint-columns-list-actions">
                      <button
                        ...
                        onClick={() =>
                          setFormData({ ...formData, columns: table.columns.map((c) => c.id) })
                        }
                      >
                        Select all
                      </button>
              ...
              {formData.type === "foreign_key" && (
                <>
                  ...
                  {formData.parentTable && (
                    <div className="constraint-form-field">
                      <label className="constraint-form-label">
                        Parent columns — select one or more
                      </label>
                      <div
                        className="constraint-columns-list"
                        role="group"
                        aria-label="Select parent columns"
                      >
                        <div className="constraint-columns-list-actions">
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              const parentCols =
                                allTables.find((t) => t.id === formData.parentTable)?.columns || [];
                              setFormData({
                                ...formData,
                                parentColumns: parentCols.map((c: UnityColumn) => c.id),
                              });
                            }}
                          >
                            Select all
              ...
              {formData.type === "check" && (
                <div className="constraint-form-field">
                  ...
                  <VSCodeTextField
                    ...
                    style={{ width: "100%" }}
                    onInput={(e) =>
                      setFormData({ ...formData, expression: (e.target as HTMLInputElement).value })
                    }
                  />
              ...
              <p className="hint" style={{ marginTop: "16px", fontStyle: "italic" }}>
                Note: Unity Catalog constraints are informational only (not enforced). They are used
                for query optimization and documentation purposes.
              </p>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setDropDialog(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="warning-text">
              Are you sure you want to drop constraint{" "}
              <strong>{dropDialog.name || "unnamed"}</strong>?
            </p>
            <div className="modal-buttons">
              <VSCodeButton onClick={handleDropConstraint}>Drop</VSCodeButton>
        ...
        <div className="modal-overlay" onClick={() => setPkWarningDialog(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ lineHeight: "1.6", marginBottom: "16px" }}>
              This table already has a PRIMARY KEY constraint. Databricks allows only one PRIMARY
              KEY per table.
            </p>
            <p style={{ lineHeight: "1.6", marginBottom: "16px" }}>
              To add more columns to the existing PRIMARY KEY, use the edit button (
              <i className="codicon codicon-edit" style={{ fontSize: "12px" }}></i>) next to the
              constraint.
            </p>
            <p style={{ lineHeight: "1.6" }}>
              To create a new PRIMARY KEY with different columns, first drop the existing constraint
              using the delete button (
              <i className="codicon codicon-trash" style={{ fontSize: "12px" }}></i>
              ).
            </p>
            <div className="modal-buttons">
              <VSCodeButton onClick={() => setPkWarningDialog(false)}>OK</VSCodeButton>
            </div>
          </div>
        </div>
      )}
