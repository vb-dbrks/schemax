...
| Command | Description |
|---------|-------------|
| `schemax sql [--output FILE] [--target ENV]` | Generate SQL migration from changelog (optional env for catalog mapping). |
...
| Command | Description |
|---------|-------------|
| `schemax apply --target ENV --profile PROFILE --warehouse-id ID [--dry-run] [--no-interaction] [--auto-rollback]` | Execute SQL against target; records deployment. |
