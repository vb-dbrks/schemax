...
export function ImportAssetsPanel({
  ...
  return (
    <div className="modal-overlay" onClick={() => !isRunning && onClose()}>
      <div
        className="modal-content import-assets-panel"
      ...
      >
        ...
        <div className="modal-body">
          <p className="section-description">
            Import objects from your provider into SchemaX. Start with dry-run to preview
            operations.
          </p>
            ...
            >
              ...
              <div className="modal-field">
                ...
                <VSCodeDropdown
                  ...
                  onInput={(e) =>
                    setSqlMode(
                      ((e.target as HTMLInputElement)?.value ?? "diff") as "diff" | "replace"
                    )
                  }
                  disabled={isRunning}
                >
              ...
              <div className="modal-field">
                <FieldLabel
                  text="Target environment (optional)"
                  help="Used for catalog mapping consistency; can be left as default."
                />
                <VSCodeDropdown
                  value={sqlTarget}
            ...
            <div className={`import-progress ${progress?.level || "info"}`}>
              ...
              <div className="import-progress__bar">
                <div
                  ...
                  style={{
                    width: `${Math.max(0, Math.min(100, progress?.percent ?? (isRunning ? 5 : 0)))}%`,
                  }}
                />
              </div>
              <p className="import-progress__message">{progress?.message || "Running import..."}</p>
            </div>
          )}

          {result && (
            <div
              className={`import-result ${result.cancelled ? "warning" : result.success ? "success" : "error"}`}
            >
              <strong>
                {result.cancelled
                  ? "Import cancelled"
