...
# Apply a full config from JSON or stdin
schemax naming apply --json '{"strictMode": true, "table": {"pattern": "^[a-z][a-z0-9_]*$", "enabled": true}}' [workspace]
schemax naming show --json | schemax naming apply --stdin [workspace]
...
cd /path/to/source && schemax naming show --json | \
  schemax naming apply --stdin /path/to/target
```
