...
The same [authentication](/docs/guide/authentication/) that works locally (Databricks SDK, profiles, or env vars) works in CI/CD: use environment variables or a profile supplied from a pipeline secret.
...
- **Rollback** — `schemax rollback` (partial or to a snapshot) with appropriate flags; see [CLI Reference](/docs/reference/cli/).
...
- [Authentication](/docs/guide/authentication/) — Databricks SDK, profiles, same auth locally and in CI/CD
