...
- **Coverage checklist**: For release-ready test coverage, every scenario in this guide should have a corresponding passing test or CI/smoke step. See [Test Coverage Checklist](/reference/test-coverage-checklist) for a scenario-by-scenario checklist aligned with this document.
