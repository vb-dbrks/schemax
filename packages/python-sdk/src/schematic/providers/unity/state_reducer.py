...
def apply_operation(state: UnityState, op: Operation) -> UnityState:
    ...
    op_type = op.get("op", "").replace("unity.", "")
    ...
    # View operations
            ...
            for schema in catalog.schemas:
                if schema.id == op["payload"]["schemaId"]:
                    view = UnityView(
                        ...
                        comment=op["payload"].get("comment"),
                        dependencies=op["payload"].get("dependencies"),
                        extracted_dependencies=op["payload"].get("extractedDependencies"),
                        tags={},
                        properties={},
                    )
        ...
        view_opt = _find_view(new_state, op["target"])
        if view_opt:
            view_opt.name = op["payload"]["newName"]

    elif op_type == "drop_view":
        for catalog in new_state.catalogs:
            for schema in catalog.schemas:
                schema.views = [v for v in schema.views if v.id != op["target"]]

    elif op_type == "update_view":
        view_opt = _find_view(new_state, op["target"])
        if view_opt:
            if "definition" in op["payload"]:
                view_opt.definition = op["payload"]["definition"]
            if "dependencies" in op["payload"]:
                view_opt.dependencies = op["payload"].get("dependencies")
            if "extractedDependencies" in op["payload"]:
                view_opt.extracted_dependencies = op["payload"].get("extractedDependencies")

    elif op_type == "set_view_comment":
        view_opt = _find_view(new_state, op["payload"]["viewId"])
        if view_opt:
            view_opt.comment = op["payload"]["comment"]

    elif op_type == "set_view_property":
        view_opt = _find_view(new_state, op["payload"]["viewId"])
        if view_opt:
            view_opt.properties[op["payload"]["key"]] = op["payload"]["value"]

    elif op_type == "unset_view_property":
        view_opt = _find_view(new_state, op["payload"]["viewId"])
        if view_opt and op["payload"]["key"] in view_opt.properties:
            del view_opt.properties[op["payload"]["key"]]
