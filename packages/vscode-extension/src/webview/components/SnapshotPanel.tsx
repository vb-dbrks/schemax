    ...
    <div className="snapshot-panel">
      <div className="snapshot-header" onClick={() => setExpanded(!expanded)}>
        ...
        {uncommittedOps.length > 0 && (
          <span
            className="uncommitted-badge"
            title={`${uncommittedOps.length} uncommitted operations`}
          >
            ...
            <div className="snapshot-timeline">
              {snapshots
                ...
                .map((snapshot, index) => {
                  ...
                  const deployments =
                    project.deployments?.filter((d) => d.snapshotId === snapshot.id) || [];
          ...
          {uncommittedOps.length > 0 && (
            <div className="uncommitted-changes">
              ...
              <div className="uncommitted-content">
                ...
                <div className="uncommitted-ops">
                  {uncommittedOps.length} uncommitted operations
                </div>
                <div className="uncommitted-hint">Create a snapshot to save these changes</div>
              </div>
            </div>
          )}
        </div>
