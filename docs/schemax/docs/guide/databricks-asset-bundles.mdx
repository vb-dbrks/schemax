...
| File | Purpose |
|---|---|
| `resources/schemax.yml` | DAB resource definition: a job that runs `schemax apply` with the right environment variables |
| `resources/schemax_deploy.py` | A small Python script executed by the job; it installs `schemaxpy` and runs the apply |
...
3. **Job execution** -- When the job runs (either via `databricks bundle run` or a manual trigger), it:
   - Spins up an environment with `schemaxpy>=0.2.8` installed.
   - Executes `resources/schemax_deploy.py`.
   - The deploy script runs `schemax apply --no-interaction`, using the environment variables set in the job definition.
   - `SCHEMAX_ENVIRONMENT` is set to `${bundle.target}` (e.g., `dev`, `prod`), so the correct SchemaX environment configuration is used.
   - `SCHEMAX_WAREHOUSE_ID` tells SchemaX which SQL warehouse to use for executing DDL.
