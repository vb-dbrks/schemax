...
4. Choose the operation:
   - **Add grant** — Enter principal (user, group, or service principal) and privileges (comma-separated). The same grant is applied to all securables in scope (catalog, schemas, tables, views, volumes, functions, materialized views as applicable).
   ...
   - **Add table tag** — Set a tag name and value on each table in scope (views are not currently included).
...
- **Apply:** `schemax apply --target ENV` runs those statements against the target. Grant operations are part of **governance**; they are included when the environment’s [deployment scope](/docs/guide/environments-and-scope) includes **Governance** (default: all categories).
...
- **Governance-only deployments:** If you only want to push grant (and other governance) changes without structural DDL, use [Environments and deployment scope](/docs/guide/environments-and-scope) and set **Governance** as the only managed category for that environment.
- **Existing catalogs:** When catalogs already exist and you use [existing objects](/docs/guide/environments-and-scope#existing-catalogs), SchemaX still emits grants for those catalogs (and their schemas/tables/views) as long as Governance is in scope.

For rollback and deployment workflows, see [Workflows](/docs/reference/workflows).
