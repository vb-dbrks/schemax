...
Use the CLI with `--no-interaction` so the pipeline does not prompt. Configure authentication via environment variables or a profile supplied from a pipeline secret. See [Git and CI/CD setup](/guide/git-and-cicd-setup/) for Azure DevOps and GitHub Actions examples. The same Databricks auth (SDK, profiles) works locally and in CI/CD.
...
Yes. The CLI is a standard Python CLI; you can call it from scripts, cron, or other automation. For example: `schemax sql --target dev --output migration.sql` then run the SQL with your own runner, or `schemax apply --target dev --no-interaction` from a pipeline. See [Git and CI/CD setup](/guide/git-and-cicd-setup/).
