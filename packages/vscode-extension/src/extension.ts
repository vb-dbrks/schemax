...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Register commands
  const openDesignerCommand = vscode.commands.registerCommand("schemax.openDesigner", () =>
    openDesigner(context)
  );

  const showLastOpsCommand = vscode.commands.registerCommand("schemax.showLastOps", () =>
    showLastOps()
  );

  const createSnapshotCommand = vscode.commands.registerCommand("schemax.createSnapshot", () =>
    createSnapshotCommand_impl()
  );

  const generateSQLCommand = vscode.commands.registerCommand("schemax.generateSQL", () =>
    generateSQLMigration()
  );

  const importAssetsCommand = vscode.commands.registerCommand("schemax.importAssets", () =>
  ...
  const schemaxTree = vscode.window.registerTreeDataProvider(
    "schemax.welcome",
    schemaxTreeProvider
  );
...
async function executeImport(
  ...
  if (isImportFromSql(request)) {
    importArgs = ["import", "--from-sql", request.fromSql.sqlPath, "--mode", request.fromSql.mode];
...
async function runImportWithRequest(
  ...
  if (result.cancelled) {
    ...
    vscode.window.showInformationMessage(
      dryRun ? "SchemaX import dry-run completed" : "SchemaX import completed successfully"
    );
...
const ENVIRONMENT_PRESETS: EnvironmentPreset[] = [
  {
    ...
    description:
      "✅ Manual changes allowed, ✅ Direct changelog deployment, ✅ Auto-creates catalog",
    config: {
      allowDrift: true,
...
async function configureCustomEnvironments(
  ...
  while (true) {
    ...
    customEnvironments[envName] = {
      ...
      description:
        description || `${envName.charAt(0).toUpperCase() + envName.slice(1)} environment`,
...
async function promptForProjectSetup(
  ...
  const useDefaultEnvs = await vscode.window.showQuickPick(
    [
      {
        label: "Use default environments",
        description: "dev, test, prod with standard catalog naming",
        value: true,
      },
      {
        label: "Configure custom environments",
        description: "Manually configure environment settings",
        value: false,
      },
    ],
    {
...
async function openDesigner(context: vscode.ExtensionContext) {
  ...
  async function detectStaleSnapshots(workspacePath: string): Promise<StaleSnapshot[]> {
    ...
    const envelope = await pythonBackend.runJson<{
      stale?: StaleSnapshot[];
      data?: { stale_snapshots?: StaleSnapshot[] };
    }>("snapshot.validate", ["snapshot", "validate"], workspacePath);
    ...
    try {
      ...
      try {
        ...
        if (conflictFiles.length > 0) {
          ...
          outputChannel.appendLine(
            `[SchemaX] - Rebase conflict detected: ${latestConflictFile[0]}`
          );
        }
      } catch {
  ...
  const conflictsPattern = new vscode.RelativePattern(workspaceFolder, ".schemax/conflicts/*.json");
  ...
  const snapshotsPattern = new vscode.RelativePattern(workspaceFolder, ".schemax/snapshots/*.json");
    ...
    async (message) => {
      ...
      switch (message.type) {
        ...
        case "load-project": {
          try {
            outputChannel.appendLine(
              `[SchemaX] Loading project from: ${workspaceFolder.uri.fsPath}`
            );
            ...
            outputChannel.appendLine(
              `[SchemaX] - Latest snapshot: ${project.latestSnapshot || "none"}`
            );
            ...
            if (state && "catalogs" in state) {
              outputChannel.appendLine(
                `[SchemaX] - Catalogs: ${(state as CatalogState).catalogs?.length ?? 0}`
              );
            }
            ...
            try {
              ...
              if (conflictFiles.length > 0) {
                ...
                const latestConflictFile = conflictFiles.sort((a, b) =>
        ...
        case "show-conflict-details": {
          ...
          if (
            conflictInfo &&
            conflictInfo.conflicting_operations &&
            conflictInfo.conflicting_operations.length > 0
          ) {
            ...
            const detailMessage =
              `Snapshot: ${conflictInfo.snapshot_version}\n` +
            ...
            vscode.window
              .showWarningMessage("⚠️ Snapshot Rebase Conflict", {
                modal: true,
        ...
        case "show-stale-snapshot-details": {
          ...
          if (staleSnapshots && staleSnapshots.length > 0) {
            const detailLines = staleSnapshots
              .map((snap: StaleSnapshot) => {
                return (
            ...
            const detailMessage =
              `Found ${staleSnapshots.length} stale snapshot(s):\n\n` +
              detailLines +
              "\n\n" +
              `Resolution:\n` +
              `Run the following command(s) in terminal:\n` +
              staleSnapshots
                .map((snap: StaleSnapshot) => `  schemax snapshot rebase ${snap.version}`)
                .join("\n");

            vscode.window
              .showWarningMessage("⚠️ Stale Snapshots Detected", {
                modal: true,
                detail: detailMessage,
              })
              .then(() => {
        ...
        case "open-docs": {
          try {
            ...
            if (!docPath) {
              vscode.window.showWarningMessage(
                "SchemaX: Documentation path or URL was not provided."
              );
              break;
            }
...
async function createSnapshotCommand_impl() {
  ...
  try {
    ...
    if (uncommittedOpsCount === 0) {
      ...
      vscode.window.showInformationMessage(
        "No changes to snapshot. All operations are already included in the last snapshot."
      );
      return;
    }
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        outputChannel.appendLine(
          `[SchemaX] - Snapshot file: ${updatedProject.snapshots[updatedProject.snapshots.length - 1].file}`
        );
...
async function generateSQLMigration() {
  ...
  try {
    ...
    vscode.window.showInformationMessage(`SQL migration generated: ${filename}`);
