...
This document describes how to run tests for the SchemaX project.
...
The `test_apply_command.py` test suite ensures the `schemax apply` command works correctly in both interactive and non-interactive modes. This is critical for CI/CD pipelines where user prompts would cause the command to hang.
...
**Issue**: `ModuleNotFoundError: No module named 'schemax'`  
