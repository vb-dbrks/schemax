"""Live integration tests: function (import + apply/rollback)."""
...
def test_live_import_sees_function(tmp_path: Path) -> None:
    """Live: Seed UC objects fixture, init, import; assert state has function; validate and sql."""
    config = require_live_command_tests()
    suffix = make_suffix(config)
    workspace = make_workspace(tmp_path, "uc_func", suffix)
...
@pytest.mark.integration
def test_live_e2e_apply_function(tmp_path: Path) -> None:
    """Live E2E: Preseed, import adopt-baseline, add function, apply, rollback."""
    config = require_live_command_tests()
    suffix = make_suffix(config)
    workspace = make_workspace(tmp_path, "apply_func", suffix)
    logical_catalog = f"logical_func_{suffix}"
    physical_catalog = f"{config.resource_prefix}_apply_func_{suffix}"
    tracking_catalog = f"{config.resource_prefix}_track_func_{suffix}"
    schema_name = "core"
