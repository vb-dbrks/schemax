...
Complete guide to get started with SchemaX — both the VS Code extension and Python SDK/CLI. See [Prerequisites](/guide/prerequisites/) for what you need before starting.
...
4. **Apply (or run in pipeline)** — Run `schemax apply --target <env>` to execute the SQL against Databricks, or run the same in CI/CD (see [Git and CI/CD setup](/guide/git-and-cicd-setup/)).
5. **Create a snapshot (optional)** — Use **SchemaX: Create Snapshot** or `schemax snapshot create` to version your state.

SchemaX can run in **full mode** (create and manage catalogs, schemas, tables, and governance) or **governance-only mode** (comments, tags, grants, row filters, column masks on existing objects). See [Environments and deployment scope](/guide/environments-and-scope/) for how to configure this.
...
The SDK is published on [PyPI](https://pypi.org/project/schemaxpy/). For a development install (contributing to SchemaX itself), see the [Development guide](/guide/development/).
...
See [Git and CI/CD setup](/guide/git-and-cicd-setup/) for pipeline examples.
...
See [Git and CI/CD setup](/guide/git-and-cicd-setup/) for pipeline examples, or continue below for a basic GitHub Actions template.
...
For detailed pipeline setup including Azure DevOps, see [Git and CI/CD setup](/guide/git-and-cicd-setup/).
...
2. **Environments & scope**: See [Environments and deployment scope](/guide/environments-and-scope) for governance-only mode and existing catalogs.
3. **Grants**: See [Unity Catalog grants](/guide/unity-catalog-grants) for managing GRANT/REVOKE on catalogs, schemas, tables, and views.
4. **Read Architecture**: See [Architecture](/guide/architecture).
5. **Project Lifecycle & Workflows**: See [Workflows](/reference/workflows) for single/multi-dev, greenfield/brownfield, and rollback timelines.
