...
def test_live_import_sees_volume(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert any(v.get("name") == "managed_vol" for v in volumes), f"Volume managed_vol not in state: {[v.get('name') for v in volumes]}"
...
def test_live_e2e_apply_volume(tmp_path: Path) -> None:
    ...
    write_project_env_overrides(workspace, top_level_name=tracking_catalog, catalog_mappings={logical_catalog: physical_catalog})
    ...
    try:
        ...
        preseed = preseed_catalog_schema(executor, config, physical_catalog=physical_catalog, tracking_catalog=tracking_catalog, schema_name=schema_name, managed_root=managed_root, clear_changelog_in=workspace)
        assert preseed.status in {"success", "partial"}, f"Preseed failed: {preseed.status}"
        import_result = invoke_cli("import", "--target", "dev", "--profile", config.profile, "--warehouse-id", config.warehouse_id, "--catalog", physical_catalog, "--catalog-map", f"{logical_catalog}={physical_catalog}", "--adopt-baseline", str(workspace))
        assert import_result.exit_code == 0, import_result.output
        ...
        append_ops(workspace, [
            builder.add_table(table_id, table_name, schema_id, "delta", op_id=f"op_table_vol_{suffix}"),
            builder.add_column(f"col_id_vol_{suffix}", table_id, "event_id", "BIGINT", nullable=False, comment="Event id", op_id=f"op_col_id_vol_{suffix}"),
            builder.add_column(f"col_val_vol_{suffix}", table_id, "event_type", "STRING", nullable=True, comment="Event type", op_id=f"op_col_val_vol_{suffix}"),
            builder.add_volume(vol_id, volume_name, schema_id, "managed", comment="E2E volume", op_id=f"op_vol_{suffix}"),
        ])
        assert invoke_cli("snapshot", "create", "--name", "Volume delta", "--version", "v0.2.0", str(workspace)).exit_code == 0
